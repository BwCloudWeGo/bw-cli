...
当前代码已经支持 `bw-cli service <service> --table <table>` 绑定既有业务表：命令会读取 `configs/config.yaml` 中的数据库配置，读取指定表的真实字段，并按这些字段生成服务结构。`--table` 不再要求表字段包含默认示例字段，例如 `description`。生成后的 Gorm repository 会使用传入表名，服务启动入口会跳过 `AutoMigrate`，避免修改既有表结构。
