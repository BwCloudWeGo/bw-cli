...
2. `MongoDB 从 0 到 1 教学教程` - 28 edges
3. `Config` - 26 edges
4. `exists()` - 25 edges
5. `/graphify` - 22 edges
6. `FromGRPC()` - 19 edges
7. `工具组件总览与调用流程` - 19 edges
8. `NoteResponse` - 18 edges
9. `Config` - 18 edges
10. `serviceTemplateData` - 17 edges
...
Cohesion: 0.11
Nodes (21): Engine, Clients, Engine, Logger, MiddlewareConfig, allowAllOrigins(), CORS(), DefaultCORSConfig() (+13 more)
...
Cohesion: 0.13
Nodes (18): Context, CreateNoteRequest, GetNoteRequest, GetUserRequest, LoginRequest, NoteResponse, PublishNoteRequest, RegisterRequest (+10 more)
...
Cohesion: 0.22
Nodes (11): Add a URL and Watch a Folder, graphify add, Non-code Changes Need Manual Update, Watch Mode, CLAUDE.md Integration, Commit Hook and Native CLAUDE.md Integration, Post-commit Auto-rebuild Hook, Cluster-only Rerun (+3 more)
...
Cohesion: 0.05
Nodes (38): 10. 公共组件, 11.1 bw-cli: command not found, 11.2 protoc-gen-go: program not found, 11.3 生成项目后还有旧 module 路径, 11.4 端口被占用, 11. 常见问题, 12. 更多文档, 1. 你可以用它做什么 (+30 more)
...
Cohesion: 0.09
Nodes (28): RouterGroup, RouterGroup, JWTConfig, RouterGroup, Clients, Engine, Logger, MiddlewareConfig (+20 more)

### Community 78 - "Community 78"
Cohesion: 0.06
Nodes (32): 10. Redis：`pkg/redisx`, 11. Elasticsearch：`pkg/esx`, 12. Kafka：`pkg/kafkax`, 13.1 通用配置, 13.2 MinIO 配置, 13.3 阿里云 OSS 配置, 13.4 七牛云 Kodo 配置, 13.5 腾讯云 COS 配置 (+24 more)

### Community 79 - "Community 79"
Cohesion: 0.06
Nodes (31): 10. 单元测试怎么写, 11. 本地真实 MongoDB 联调, 12.1 在 handler 里直接调用 MongoDB, 12.2 在 service 里直接 new DocumentStore, 12.3 把 bson tag 写到业务实体上, 12.4 忘记处理 ErrNotFound, 12. 常见错误, 13. 推荐落地清单 (+23 more)

### Community 80 - "Community 80"
Cohesion: 0.07
Nodes (28): 10. 开发流程, 11. 测试与质量门禁, 12. 命名与代码约定, 13. Git 约定, 14. 项目资料入口, 15. 当前已知注意点, 1. 项目定位, 2. 技术栈 (+20 more)

### Community 81 - "Community 81"
Cohesion: 0.08
Nodes (23): 10. 在服务启动时创建 MongoDB client, 11. DDD 中 MongoDB 代码应该放在哪里, 12. 定义文档模型, 13. 在 repo 层适配 BSON 结构, 14. 实现模型转换, 15. 创建索引, 16. 保存文档, 17. 按 note_id 查询 (+15 more)

### Community 82 - "Community 82"
Cohesion: 0.12
Nodes (15): entity, Gateway 分层, handler, model, repo, service, 中间件, 公共包 (+7 more)

### Community 83 - "Community 83"
Cohesion: 0.15
Nodes (15): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Semantic Extraction (+7 more)

### Community 84 - "Community 84"
Cohesion: 0.14
Nodes (14): 6.10 Elasticsearch 配置, 6.11 Kafka 配置, 6.12 CORS 配置, 6.13 JWT 配置, 6.1 服务名配置, 6.2 HTTP 配置, 6.3 gRPC 配置, 6.4 MySQL 默认配置 (+6 more)

### Community 85 - "Community 85"
Cohesion: 0.15
Nodes (12): entity 层, gateway 层, handler 层, model 层：数据库结构, Order 服务开发说明, repo 层：数据库在哪里操作，如何操作, service 层, 启动 (+4 more)

### Community 86 - "Community 86"
Cohesion: 0.18
Nodes (10): Bulk 写入 ES, Elasticsearch 调用示例, MySQL 同步到 ES, 初始化, 增量扫描, 数据结构, 模糊搜索和高亮, 游标保存 (+2 more)

### Community 87 - "Community 87"
Cohesion: 0.20
Nodes (9): Adding A Service, Build And Run, Conventions, Git Style, Project Instructions, Project Structure, Service Layering, Tech Stack (+1 more)
...
Cohesion: 0.22
Nodes (8): 1. 配置来源规则, 2. Nacos 开关配置, 3. 在 Nacos 中创建配置, 4. 启用 Nacos, 5. 本地开发和生产建议, 6. 不需要修改 Config 结构体, 7. 排查, Nacos 配置中心使用说明

### Community 90 - "Community 90"
Cohesion: 0.22
Nodes (8): 11. 发布公共包给其他项目使用, 2.1 确认 go.mod module, 2.2 提交到 Git 仓库, 2.3 验证远程安装命令, 2. 作为脚手架维护者：发布 bw-cli, 3. 作为脚手架使用者：安装 bw-cli, 5. 初始化生成后的项目, 使用说明

### Community 91 - "Community 91"
Cohesion: 0.22
Nodes (9): 10.1 生成后已经具备什么, 10.2 每一层怎么写，为什么这么写, 10.3 entity 层：写业务核心, 10.4 model 层：写数据库结构, 10.5 service 层：写业务流程, 10.6 repo 层：数据库在哪里操作，如何操作, 10.7 gateway 层：HTTP 调用已经接好, 10.7 handler 层：协议转换 (+1 more)

### Community 92 - "Community 92"
Cohesion: 0.25
Nodes (7): 创建支付, 初始化, 同步回调验签, 异步通知回调, 支付宝支付调用示例, 退款, 配置

### Community 93 - "Community 93"
Cohesion: 0.25
Nodes (8): 6.1 插入一条文档, 6.2 查询全部文档, 6.3 按字段查询, 6.4 更新文档, 6.5 不存在则插入, 6.6 删除文档, 6.7 统计数量, 6. 用 mongosh 学会基础 CRUD
...
Cohesion: 0.29
Nodes (7): 24.1 server selection timeout, 24.2 authentication failed, 24.3 no documents in result, 24.4 cursor 泄漏, 24.5 每次请求都创建 client, 24.6 查询慢, 24. 常见错误排查

### Community 96 - "Community 96"
Cohesion: 0.29
Nodes (7): 8.1 干净项目：健康检查, 8.2 演示项目：注册用户, 8.3 登录用户, 8.4 查询用户, 8.5 创建笔记, 8.6 发布笔记, 8. 调用接口验证

### Community 97 - "Community 97"
Cohesion: 0.33
Nodes (6): 12.1 bw-cli: command not found, 12.2 go install 远程安装失败, 12.3 生成项目后 protobuf panic, 12.4 演示项目 gateway 调用 user/note 失败, 12.5 JWT 返回 invalid token, 12. 常见问题

### Community 98 - "Community 98"
Cohesion: 0.33
Nodes (6): Existing Graph Fast Path, Existing Graph Query Flow, Query, Path, and Explain Flows, BFS Traversal Mode, DFS Traversal Mode, Query, Path, Explain

### Community 99 - "Community 99"
Cohesion: 0.40
Nodes (5): 25. 课堂练习, 练习 1：命令行 CRUD, 练习 2：创建索引并分析查询, 练习 3：在 Go 中保存笔记文档, 练习 4：实现分页查询

### Community 100 - "Community 100"
Cohesion: 0.40
Nodes (5): 7.1 创建唯一索引, 7.2 创建组合索引, 7.3 查看索引, 7.4 看查询是否命中索引, 7. 理解索引

### Community 101 - "Community 101"
Cohesion: 0.40
Nodes (5): 1.1 检查 Go, 1.2 检查 protoc, 1.3 安装 Go 的 proto 插件, 1.4 验证脚手架自身, 1. 准备环境

### Community 102 - "Community 102"
Cohesion: 0.40
Nodes (5): 4.1 生成干净业务项目, 4.2 生成演示项目, 4.3 从本地脚手架目录生成, 4.4 bw-cli 生成时做了什么, 4. 生成新项目

### Community 103 - "Community 103"
Cohesion: 0.40
Nodes (5): 7.1 演示项目：启动 user-service, 7.2 演示项目：启动 note-service, 7.3 启动 gateway, 7.4 查看日志, 7. 启动服务

### Community 104 - "Community 104"
Cohesion: 0.50
Nodes (4): 9.1 HTTP 入参在哪里, 9.2 路由在哪里, 9.3 业务服务怎么看, 9. 代码结构怎么读
...
Cohesion: 0.67
Nodes (3): 3.1 检查 Go, 3.2 检查 bw-cli 项目, 3. 准备环境
...
- **Why does `setDefaults()` connect `Config Loading` to `Community 77`, `HTTP Gateway`?**
...
- **Should `App Error Handling` be split into smaller, more focused modules?**
